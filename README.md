...
Lucet has the following options that can be adjusted in the `rel/files/sys.config`:

Parameter | Description | Example
----------|-------------|-----------
ssh_port | List port for ssh connections | 22333
dobby_node | Erlang node name that runs Dobby | 'dobby@127.0.0.1'

## SSH

Lucet includes an ssh daemon for command shell connections. In order to connect
to lucet using an ssh client, you must create ssh keys prior to starting lucet.
To create the keys follow the instructions in
[priv/system_dir/README.md](priv/sytem_dir/README.md)
and [priv/user_dir/README.md](priv/user_dir/README.md). When the keys
are correctly created they will be copied to the release tree
every time it's generated.

# Building and running

To build the release run `make rel`.

Every time you change a parameter in the `rel/files/sys.config` or the
keys in the `priv/system_dir` or `priv/user_dir` run `make overlay`
to update the release.

To start the release invoke `rel/lucet/bin/lucet console`.
